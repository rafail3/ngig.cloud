...
// tokens as the shell, themed for light/dark by the globals.css remap. The
// blocks are the shared Skeleton so every loading surface pulses alike.
    ...
    <div className="flex min-h-screen flex-col bg-zinc-950 text-zinc-50">
      ...
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-3 border-b border-zinc-900 bg-zinc-950/95 px-3 backdrop-blur sm:px-5">
        <div className="flex items-center gap-2">
          <Skeleton className="h-9 w-28 rounded bg-zinc-900 sm:h-10 sm:w-32" />
        </div>
          ...
          <nav className="flex-1 space-y-1 px-3 py-4">
            {Array.from({ length: 4 }).map((_, i) => (
              ...
              >
                ...
                <Skeleton className="h-4 w-24 rounded bg-zinc-900" />
              </div>
            ))}
          </nav>
        </aside>
        ...
        <main className="min-w-0 flex-1">
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
              {Array.from({ length: 4 }).map((_, i) => (
                <Skeleton
                  key={i} className="h-24 rounded-2xl border border-zinc-800 bg-zinc-900/40" />
              ))}
            </div>
