...
function DeleteButton({ id, title, full = false }: { id: string; title: string; full?: boolean }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <button
      ...
      {open && <ConfirmDelete id={id} title={title} onClose={() => setOpen(false)} />}
    </>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl">
        ...
        <h3 className="mt-4 text-base font-semibold text-zinc-100">Ștergi anunțul?</h3>
        <p className="mt-1.5 text-sm text-zinc-400">
          ...
          <div key={a.id} className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-xs">
              <div>
                ...
                <dd className="tabular-nums text-zinc-300">{a.recipient_count}</dd>
              </div>
              <div>
                <dt className="text-zinc-500">Trimis</dt>
                <dd className="text-zinc-300">{fmt(a.created_at)}</dd>
              </div>
            </dl>
